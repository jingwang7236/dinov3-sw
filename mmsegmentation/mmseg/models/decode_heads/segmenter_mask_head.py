...
@MODELS.register_module()
class HCSSegmenterMaskTransformerHead(BaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        x = self._transform_inputs(inputs)
        b, c, cin, h, w = x.shape
        x = x.permute(0, 3, 4, 1, 2).contiguous().view(b, -1, c)   # [B, N, C]
        ...
        # Compute similarity: [B, N, num_classes]
        masks = patches @ cls_seg_feat.transpose(1, 2)
        masks = self.mask_norm(masks)
        ...
        # Reshape masks for HCSChannelViT: split N -> (in_ch, h*w)
        # masks: [B, N, num_classes] -> [B, num_classes, N]
        masks = masks.permute(0, 2, 1).contiguous()
        # view -> [B, num_classes, cin, h*w]
        masks = masks.view(b, self.num_classes, cin, h * w)
        # Aggregate channel dimension (e.g., mean)
        masks = masks.mean(dim=2) # [B, num_classes, h*w]     # INTO: 加一个mean，处理channel维度
        masks = masks.view(b, self.num_classes, h, w)
