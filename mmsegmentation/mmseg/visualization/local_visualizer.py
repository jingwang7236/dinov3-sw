...
@VISUALIZERS.register_module()
class CloudSegLocalVisualizer(SegLocalVisualizer):
    ...
    def add_datasample(
        ...
        if gt_img_data is not None and pred_img_data is not None:
            ...
            # drawn_img = pred_img_data
            drawn_img = np.concatenate((image, pred_img_data), axis=1)

        out_file = f'work_dirs/cloud_seg/vis_result/{name}_{step}.png'
