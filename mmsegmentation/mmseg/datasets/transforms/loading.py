...
# @TRANSFORMS.register_module()
# class LoadRSImageFromFileWithChannelInfo(BaseTransform):
#     """Load a Remote Sensing mage from file and modified 'channels'.
...
@TRANSFORMS.register_module()
class LoadImageFromFileWithChannelInfo(BaseTransform):
...
@TRANSFORMS.register_module()
class LoadMSImageFromFileWithSubBands(BaseTransform):
    """Load a Remote Sensing mage from file.
    ...
    def __init__(self, to_float32: bool = True, select_bands=None):
        self.to_float32 = to_float32
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        if ds is None:
            raise Exception(f'Unable to open file: {filename}')
        try:
            if self.select_bands is None:
                arr = ds.ReadAsArray()
            else:
                nb = ds.RasterCount
                bands = list(self.select_bands)
