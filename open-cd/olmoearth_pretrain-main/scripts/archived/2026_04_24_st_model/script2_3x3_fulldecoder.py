"""ST model with windowed attention + single bandset + random band dropout (no S1) + random time with decode masking + cosine similarity loss.

- ST model with windowed_attention_size=3
- Single bandset S2 (all 12 bands) / Landsat (all 11 bands)
- Random band dropout (rate ~ Uniform(0, 0.2)) on S2 and Landsat only (no S1 dropout)
- Random time with decode masking
- Cosine similarity loss (no patch discrimination, no contrastive)
...
from olmoearth_pretrain.nn.st_model import STEncoderConfig
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    WANDB_PROJECT = "2026_04_24_st_model"
