"""Add hidden layers before AND after the patch projection.

Same per-pixel hidden layer as hidden1.py (Linear(in_chans, 64) -> ReLU) before
patchification, plus one extra 768 -> 768 ReLU layer after the patch projection
(before the transformer blocks).

Adapted from hidden1.py in this directory.
"""
...
# Pre-patchification: per-pixel MLP (same as hidden1.py).
PATCH_EMBED_HIDDEN_SIZES: list[int] = [64]
# Post-projection: ReLU -> Linear(768, 768) after the patch projection.
POST_PROJ_HIDDEN_SIZES: list[int] = [768]
...
def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    ...
    encoder_config = EncoderConfig(
        ...
        post_proj_hidden_sizes=POST_PROJ_HIDDEN_SIZES,
    )
