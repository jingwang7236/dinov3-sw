"""Base script for single bandset + random band dropout (no S1) + random time with decode masking + NDVI decode-only + masked-negatives loss.
...
- NDVI added as a decode-only modality
