"""v1.2 official baseline: v1.1 (hidden patch-embed projection) + mixed 3D RoPE.

The v1.2 baseline is the winning configuration from the temporal-RoPE sweep
(project ``2026_04_22_add_hidden_layer_to_initial_projection``, run ``nd3xh7py`` /
``trope_mixed_tscale_months``): the v1.1 model plus learnable mixed 3D RoPE
(Heo et al. 2024, extended from 2D to 3D over ``(t, row, col)``) with the
temporal coordinate scaled into ~months.

This is a self-contained copy of ``scripts/official/v1_1/base.py``; the only
functional change is that the encoder and decoder use the ``rope_3d_mixed``
spatial positional encoding. Size variants (decoder depth 4 for all) live in
nano/tiny/small/large.py.
"""
...
# Mixed 3D RoPE knobs -- the winning trope_mixed_tscale_months config. The
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    WANDB_PROJECT = "2026_06_12_v1_2_size_sweep"
...
def build_size_model_config(
    ...
    """Build a v1.2 (mixed 3D RoPE) model config for the given size preset."""
...
def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    """Build the default v1.2 base model config (base encoder, decoder depth 4)."""
