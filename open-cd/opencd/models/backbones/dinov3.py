...
@MODELS.register_module()
class SiameseDinoV3Backbone(DinoV3Backbone):
    """
    Siamese version of DinoV3Backbone for Open-CD.
    直接继承 DinoV3Backbone，完全复用所有功能，只修改接口适配双输入。
    """
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

    def forward(self, img_A, img_B):
        feats1 = super().forward(img_A)
        feats2 = super().forward(img_B)
        outs = []
