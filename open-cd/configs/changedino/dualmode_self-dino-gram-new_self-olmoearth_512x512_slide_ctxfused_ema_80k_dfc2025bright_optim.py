# DFC2025 BRIGHT 前光后 SAR 变化检测 — 4 分类 + 多 trick (快速版)
# ============================================================================
# 基于 *_optim.py, 保留 native_size=384 的分辨率收益, 但关闭梯度检查点并恢复
# batch_size=8, 以大幅提速 (use_checkpoint 反向重算 transformer, 单步约 ×2 算力)。
#
# 与 _optim.py 的唯一差异:
#   - backbone_sar.use_checkpoint: True  -> False   (省 ~2× SAR 算力)
#   - train_dataloader.batch_size:  12   -> 8        (对齐原 3 天版 bs)
# 其余完全同 _optim: native 384 / stride(256,256) / 双 backbone 全冻结 / EMA /
#                    ms_inference+TTA / Lovász+Focal / 80k。
#
# 注意: 关闭 use_checkpoint 后显存上升, native 384 + bs 8 在显存不足时可能 OOM。
#       应对 (按需): 降 bs 到 4~6, 或 native 384->320, 或退回 use_checkpoint=True。
# ============================================================================
...
model = dict(
    ...
    backbone_sar = dict(
        ...
        use_checkpoint=False,           # ★ 关闭梯度检查点 (提速 ~2×), 显存上升
        modality='sentinel1',
        input_res=10,
        default_month=0,
