...
from dinov3.train.lora_utils import apply_lora_to_student, get_lora_param_groups
...
def do_train(cfg, model, resume=False):
    ...
    apply_lora_to_student(model, cfg)
    
    # 如果启用了 LoRA，使用自定义的参数组获取逻辑
    if hasattr(cfg, 'peft') and cfg.peft.enabled:
        # 注意：这里需要根据你的 lr 调度逻辑调整 base_lr
        # 假设 cfg.optim.lr 是基础学习率
        params_groups = get_lora_param_groups(model, cfg.optim.lr, cfg.optim.weight_decay)
        optimizer = build_optimizer(cfg, params_groups)
        logger.info("Using LoRA-specific parameter groups for optimizer.")
    else:
        # Raw Optimizer
