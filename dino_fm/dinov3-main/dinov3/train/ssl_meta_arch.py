...
def adapt_pretrain_chkpt_channel_3to1(state, possible_keys):
    for k in possible_keys:
        if k in state and state[k].ndim == 4 and state[k].shape[1] == 3:
            state[k] = state[k].mean(dim=1, keepdim=True)
            return k
    return None
...
class SSLMetaArch(nn.Module):
    ...
    def init_weights(self) -> None:
        ...
        if self.has_gram_teacher:
            if self.gram_ckpt is not None:
                ...
                init_fsdp_model_from_checkpoint(
                    ...
                    channel_adaptive=self.cfg.train.channel_adaptive
                )
        ...
        if self.cfg.futher_ssl.enabled:
            logger.info(f"Loading pretrain backbone to training from : {self.cfg.futher_ssl.checkpoint_path}")

            checkpoint = torch.load(self.cfg.futher_ssl.checkpoint_path, map_location='cpu')
            ...
            if self.cfg.train.channel_adaptive:
                possible_keys = [
                ...
                used = adapt_pretrain_chkpt_channel_3to1(chkpt, possible_keys)
                logger.warning(f"adapted keys: {used}")

            incomp = self.teacher.backbone.load_state_dict(chkpt, strict=False)
            missing = list(getattr(incomp, "missing_keys", []))
            unexpected = list(getattr(incomp, "unexpected_keys", []))
            logger.warning(f"teacher.backbone missing keys: {missing}")
            logger.warning(f"teacher.backbone unexpected keys: {unexpected}")

            incomp = self.student.backbone.load_state_dict(chkpt, strict=False)
            missing = list(getattr(incomp, "missing_keys", []))
            unexpected = list(getattr(incomp, "unexpected_keys", []))
            logger.warning(f"student.backbone missing keys: {missing}")
            logger.warning(f"student.backbone unexpected keys: {unexpected}")

            incomp = self.model_ema.backbone.load_state_dict(chkpt, strict=False) 
            missing = list(getattr(incomp, "missing_keys", []))
            unexpected = list(getattr(incomp, "unexpected_keys", []))
            logger.warning(f"model_ema.backbone missing keys: {missing}")
            logger.warning(f"model_ema.backbone unexpected keys: {unexpected}")
